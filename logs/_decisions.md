...
## Session 2026-05-08-002 — 2026-05-08 (Security Audit Remediation)

**Decision:** Encrypt Room database with SQLCipher 4 + Android Keystore-backed key
**Rationale:** Security audit finding #1 (HIGH) — unencrypted health data at rest is the blocking item for Play Store launch. SQLCipher 4 is the mature standard for Android local encryption; Android Keystore provides hardware-backed key storage on supported devices. Keystore key is immutable and never exported from the enclave. Existing plaintext databases auto-migrate safely using `sqlcipher_export()` on first app open (no user action required, no data loss).
**Context:** Comprehensive security audit by multiple agents identified 7 findings (1 HIGH, 4 MEDIUM, 2 LOW). DB encryption was the critical blocker. Migration uses ATTACH + export pragma (canonical SQLCipher pattern), not manual row copy.

**Decision:** Replace custom CSV parser with RFC-4180 kotlin-csv library
**Rationale:** Finding #2 (MEDIUM). Custom `.split(",")` parser breaks on quoted commas, escaped quotes, unusual line endings, huge files. kotlin-csv is pure Kotlin, zero-dependency, RFC-4180 compliant. Fuzz test suite added to catch malformed imports (46 tests covering quoted commas, CRLF/LF, edge cases, blank files, 10K rows, encoding). Enforce 5MB size limit, 10K row limit, 10 column limit at import boundary.
**Context:** Manual parser had been "fine for happy path" but is a data integrity risk. kotlin-csv 1.10.0 added to gradle/libs.versions.toml.

**Decision:** Harden GitHub Actions with SHA pinning, least-privilege permissions, dependency scanning
**Rationale:** Finding #3 (MEDIUM). All actions pinned to full commit SHAs (not mutable tags — tags can be moved by compromised upstream repos). Top-level permissions block with `contents: read` default. Replace `ubuntu-latest` with `ubuntu-24.04` (immutable runner image). Add `actions/dependency-review-action` (new vulns on PRs), CodeQL SAST (security-extended suite), `google/osv-scanner-action` (known CVE scanning), Gradle Wrapper validation (tampering detection). Create `.github/dependabot.yml` for automated Gradle + Actions updates. `build-release` job gated on all 4 security checks.
**Context:** Supply-chain hardening is necessary before public GitHub release. Dependabot configuration includes exclusions for AGP/Gradle (manual upgrade required due to build-gotcha review).

**Decision:** SECURITY.md rewritten with accurate threat model; dependency scanning proven in CI
**Rationale:** Finding #4 (MEDIUM). Original SECURITY.md made false claims about certificate pinning (misleading — no cert pinning needed for offline-first app) and claimed dependency scanning but didn't implement it. New SECURITY.md documents: SQLCipher encryption, no INTERNET permission, cleartext traffic disabled, supply-chain hardening. Dependency scanning is now live in GitHub Actions CI (dependency-review-action + OSV scanner), so the claim is backed by working code.
**Context:** Audit revealed the overclaims; they are now either removed or verified by live CI gates.

**Decision:** CSV export offers three formats: plaintext, encrypted ZIP (AES-256), minimal (date+weight only)
**Rationale:** Finding #5 (MEDIUM) + GDPR Art. 20 (data portability). Plaintext CSV is dangerous if shared (warns user). Encrypted ZIP with 12+ character password (not stored or logged) provides strong security for users who need it (CharArray handling, CRC re-verification on write). Minimal CSV removes profile data but warns users it's quasi-identifying (date+weight series + external context can re-identify). UI: radio button selector + password TextField with strength indicator + per-format warning. Uses zip4j 2.11.5 with explicit AES-256 + WinZip v2 config (no ZipCrypto fallback). Compatible with 7-Zip / WinRAR / The Unarchiver. Does NOT work on macOS Archive Utility (limitation documented).
**Context:** Finding #5 required encrypted export option. Security-auditor identified 10 threat-model findings during design review; all addressed in implementation (password handling, integrity check, dependency supply-chain audit pending, etc.).

**Decision:** Notification permission requested BEFORE reminder toggle is enabled
**Rationale:** Finding #6 (LOW). Previous flow: toggle enabled → request permission → schedule worker. If user denied permission, toggle stayed ON but notifications would never appear. New flow: toggle click → emit RequestNotificationPermission event → UI launches system dialog → if granted, enable toggle and schedule; if denied, keep toggle OFF and show dismissible message. Consistent state between toggle position and actual capability.
**Context:** Uncovered during phase 4 audit; low severity but impacts UX clarity.

**Decision:** Release signing credentials validated early (all 4 fields: storeFile, storePassword, keyAlias, keyPassword)
**Rationale:** Finding #7 (LOW). Original code only checked `storeFile` existence. New `validateReleaseSigning` Gradle task (runs before assembleRelease) validates all 4 fields non-empty. Fails fast with clear error message if any missing. Prefer CI secrets over local.properties (environment variables injected by GitHub Actions). Local fallback still supported for developer convenience.
**Context:** Prevents silent debug signing fallback that could ship a dev-signed APK to Play Store. ADR-008 documents the decision; RELEASE_SIGNING_GUIDE.md explains local and CI setup.

**Decision:** All 7 audit findings resolved in single session via multi-agent dispatch
**Rationale:** Parallel agent teams for independent work (kotlin-specialist for DB + CSV + export, devops-engineer for CI, security-auditor for docs, mobile-app-developer for UX, build-engineer for gradle). Total: 280+ unit tests (46 fuzz), BUILD SUCCESSFUL verified by agents. Single large commit `security: implement all 7 audit findings` documents all changes.
**Context:** User requested "directly solve all 7 problems" — dispatched appropriate agents in parallel waves. Cost/benefit: 3-4 hours of AI agent time, 5-6 hours of tokens, unblocks Phase 4 gate. Manual implementation would take 2-3 weeks solo.

## Session 2026-05-08-002 — 2026-05-08

**Decision:** Debug builds skip SQLCipher and use plain Room.
**Rationale:** EncryptedSharedPreferences Keystore initialization on first run blocks the main thread for 30-55 seconds, causing permanent black screen. Release builds still encrypt the database.
**Context:** First physical device test revealed the app was stuck on a black screen. Root cause traced to `EncryptedSharedPreferences.create()` blocking the main thread in `buildDatabase()`, triggered synchronously from `MainActivity.onCreate()`.

**Decision:** OSV vulnerability scan uses the reusable workflow (`osv-scanner-reusable.yml`).
**Rationale:** The direct Docker action (`osv-scanner-action/osv-scanner-action@sha`) is explicitly marked "not intended for direct use" in its own action.yml. Three iterations of flag/path fixes failed. Reusable workflow handles SARIF output, flags, and upload internally.
**Context:** OSV scan was the last persistent CI red cross after all other fixes. Took 6 commits to resolve.
