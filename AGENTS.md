# AGENTS.md — WeightFlow

This file provides guidance to Codex when working inside the WeightFlow project directory.
...
All agents live in `.Codex/agents/`.
...
### Project-local originals (`.Codex/skills/`)
| Skill | When |
|-------|------|
...
| `Codex-automation-recommender` | After Phase 1 — audit what to automate |
| `Codex-md-improver` | Keep this AGENTS.md accurate as we build |

### Marketplace skills — Android & CI (`.Codex/skills/` → symlinked)
| Skill | Phase | Source |
|-------|-------|--------|
...
`gh-cli` · `git-commit` · `git-guardrails-Codex` · `prd-to-issues` · `grill-me` · `simplify` · `improve-codebase-architecture` · `critique` · `design-an-interface`
...
| `Codex-md-management` | Update AGENTS.md with session learnings | Every /wrap |
...
Codex is a headless coding agent that runs sandboxed tasks in parallel with Codex. Use it for **isolated, well-scoped generation tasks** where you want to parallelize work.
...
| Any | Second-pass investigation when Codex is stuck | `codex:rescue` skill |
| Any | Refactor a specific file in isolation | `codex "refactor X to use Y pattern"` |

**When NOT to use Codex:** Architecture decisions, multi-file coordination, anything touching NavGraph wiring or DI root — those need Codex's full context.
...
8. /wrap → update logs/_state.md + session log + this AGENTS.md
...
Three rules in `.Codex/hookify.*.local.md` — active immediately, no restart needed:
...
1. **Git commits**: No Codex co-authorship ever. No `Co-Authored-By` trailers. Write messages manually. History was cleaned 2026-04-17.
...
- **Session end**: Always run `/wrap` before closing Codex
